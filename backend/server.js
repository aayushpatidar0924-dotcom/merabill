...
app.post("/add-bill", upload.single("billImage"), async (req, res) => {
  try {
    ...
    if (!req.file) return res.status(400).json({ success: false, message: "Bill image is required" });
...
app.get("/bills/org/:organization", async (req, res) => {
  try {
    const workers = await User.find({ organization: req.params.organization });
    ...
    res.status(500).json({ success: false, message: "Error fetching organization bills" });
  }
});
